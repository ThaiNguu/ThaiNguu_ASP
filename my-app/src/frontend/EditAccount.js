...
function EditAccount() {
  ...
  useEffect(() => {
    const fetchUser = async () => {
      try {
        ...
        if (!token) {
          setErrorMessage('Vui lòng đăng nhập để chỉnh sửa thông tin tài khoản');
          navigate('/login');
          return;
        }
        ...
        setFormData({
          ...
          Create_by: response.data.Create_by || response.data.create_by || '',
          Update_by: JSON.parse(localStorage.getItem('user'))?.id || '',
          Delete_at: response.data.Delete_at || response.data.delete_at || null,
          Delete_by: response.data.Delete_by || response.data.delete_by || '',
        });
      } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    const userData = {
      ...
      Role: formData.Role, // Preserve existing role
    ...
    try {
      ...
      let errorMsg = 'Lỗi khi cập nhật thông tin tài khoản';
              ...
              <div className="row">
                <div className="col-12 text-right">
                  ...
                  <Link className="btn btn-sm btn-info" to="/account">
                    <i className="fa fa-arrow-left"></i> Về tài khoản
                  </Link>
                </div>
              </div>
            </div>
